...
const Header = ({ cities = [], regions = [] }) => {
  ...
  const handleCityClick = (name) => {
    ...
    setStaedteOpen(false);
    navigate(`/stadte/${slug}`);
  };
  ...
  return (
    <>
            ...
            <div className="h-16 sm:h-[72px] flex items-center justify-between relative">
              ...
              <nav className="hidden lg:flex items-center gap-8" data-testid="header-left-nav">
                ...
                <div ref={staedteRef} className="relative">
                  <button onClick={() => { setStaedteOpen(!staedteOpen); setKontaktOpen(false); }} data-testid="nav-staedte"
                    className="nav-link-hover text-[14px] font-semibold tracking-[0.08em] uppercase flex items-center gap-1.5">
                    Städte <ChevronDown size={14} className={`transition-transform duration-200 ${staedteOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {staedteOpen && (
                    <div className="absolute top-full left-0 mt-3 w-52 rounded-xl overflow-hidden shadow-2xl" style={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)' }}>
                      {NRW_CITIES.map(city => (
                        <button key={city} onClick={() => handleCityClick(city)}
                          className="w-full text-left px-4 py-2.5 text-[13px] text-white/50 hover:text-white hover:bg-white/5 transition-colors flex items-center gap-2">
                          <MapPin size={12} className="opacity-40" />{city}
                        </button>
                      ))}
                      <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                        <button onClick={() => { setStaedteOpen(false); setSearchOpen(true); }}
                          className="w-full text-left px-4 py-3 text-[12px] font-semibold tracking-wide uppercase transition-colors" style={{ color: '#dc1414' }}>
                          Mehr Städte...
                        </button>
                      </div>
                    </div>
                ...
                <div ref={kontaktRef} className="relative">
                  ...
                  {kontaktOpen && (
                    <div className="absolute top-full left-0 mt-3 w-48 rounded-xl overflow-hidden shadow-2xl" style={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <Link to="/support" onClick={() => setKontaktOpen(false)}
                        className="block px-4 py-3 text-[13px] text-white/50 hover:text-white hover:bg-white/5 transition-colors">Support</Link>
                      <Link to="/inserieren" onClick={() => setKontaktOpen(false)}
                        className="block px-4 py-3 text-[13px] font-semibold transition-colors" style={{ color: '#dc1414', borderTop: '1px solid rgba(255,255,255,0.06)' }}>Inserieren</Link>
                    </div>
                  )}
                </div>
              </nav>
              ...
              <div className="hidden lg:flex items-center gap-7">
                <button onClick={() => setSearchOpen(!searchOpen)} data-testid="nav-search-toggle"
                  className="nav-link-hover text-[14px] font-semibold tracking-[0.08em] uppercase flex items-center gap-2">
              ...
              <div className="lg:hidden flex items-center gap-4">
                <button onClick={() => setSearchOpen(!searchOpen)} className="text-white/60 hover:text-white transition-colors"><Search size={20} /></button>
                <Sheet open={open} onOpenChange={setOpen}>
                  <SheetTrigger asChild><button data-testid="site-header-menu-button" className="text-white/60 hover:text-white transition-colors"><Menu size={22} /></button></SheetTrigger>
                  <SheetContent side="right" className="border-l w-72 p-0" style={{ background: '#0a0a0a', borderColor: 'rgba(220,20,20,0.08)' }}>
                    ...
                    <div className="px-4 space-y-0.5">
                      ...
                      <div className="pt-3 mt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                        <div className="px-3 pb-2 text-[9px] font-bold tracking-[0.2em] uppercase" style={{ color: 'rgba(220,20,20,0.35)' }}>Kategorien</div>
                        {SUB_NAV.map(item => (
                      ...
                      <div className="pt-3 mt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                        <div className="px-3 pb-2 text-[9px] font-bold tracking-[0.2em] uppercase" style={{ color: 'rgba(220,20,20,0.35)' }}>Städte</div>
                        {NRW_CITIES.slice(0, 6).map(city => (
                          <button key={city} onClick={() => { handleCityClick(city); setOpen(false); }} className="mobile-nav-link w-full text-left">{city}</button>
                        ))}
                      </div>
                    </div>
                  </SheetContent>
            ...
            <div className="flex items-center justify-center gap-6 sm:gap-8 lg:gap-10 py-2.5 overflow-x-auto scrollbar-hide">
              {SUB_NAV.map(item => (
                <Link key={item.name} to={`/kategorien/${item.slug}`} data-testid={`subnav-${item.slug}`}
                  className="subnav-link whitespace-nowrap text-[10px] sm:text-[11px] font-semibold tracking-[0.15em] uppercase"
      ...
      <style>{`
        .nav-link-hover { color: rgba(255,255,255,0.55); transition: color 0.3s; }
        .nav-link-hover:hover { color: #dc1414; }
        .subnav-link { transition: color 0.3s; }
        .subnav-link:hover { color: #dc1414 !important; }
        .mobile-nav-link { display:block; padding:10px 8px; font-size:12px; letter-spacing:0.08em; text-transform:uppercase; color:rgba(255,255,255,0.45); transition:color 0.15s,background 0.15s; border-radius:4px; }
